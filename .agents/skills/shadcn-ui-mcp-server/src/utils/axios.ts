...
async function getComponentDemo(componentName: string): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      `Demo for component "${componentName}" not found in v4 registry`,
    );
  }
}
...
async function getAvailableComponents(): Promise<string[]> {
  ...
  try {
    ...
    const response = await githubApi.get(
      `/repos/${REPO_OWNER}/${REPO_NAME}/contents/${basePath}/ui`,
    );
    ...
    if (error.response) {
      ...
      if (status === 403 && message.includes("rate limit")) {
        ...
        throw new Error(
          `Components directory not found. The path ${basePath}/ui may not exist in the repository.`,
        );
      } else if (status === 401) {
        throw new Error(
    ...
    return getUiLibrary() === "base"
      ? getFallbackComponentsBase()
...
async function getComponentMetadata(componentName: string): Promise<any> {
  try {
    ...
    const dependenciesMatch = componentData.match(
      /dependencies:\s*\[([^\]]*)\]/s,
    );
    const registryDepsMatch = componentData.match(
      /registryDependencies:\s*\[([^\]]*)\]/s,
    );
 ...
 */
function generateComplexBlockUsage(
  blockName: string,
  structure: any[],
): string {
...
async function getBlockCode(
  ...
  try {
    ...
    try {
      const simpleBlockResponse = await githubRaw.get(
        `/${blocksPath}/${blockName}.tsx`,
      );
    ...
    blockStructure.usage = generateComplexBlockUsage(
      blockName,
