...
export const setupHandlers = (server: Server): void => {
  ...
  server.setRequestHandler(ListResourcesRequestSchema, async (request) => {
    return await handleRequest("list_resources", request.params, async () => ({
      resources,
    }));
  });
          ...
          inputSchema: {
            ...
            properties: {
              query: {
                type: "string",
                description:
                  "Search query for theme (e.g., 'cyberpunk', 'modern')",
              },
              presetId: {
                type: "string",
              ...
              dryRun: {
                type: "boolean",
                description:
                  "If true, returns preview instead of writing files",
              },
            },
          },
          annotations: {
  ...
  server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    return await handleRequest(
      ...
      async (validatedParams: any) => {
        ...
        const promptHandler =
          promptHandlers[name as keyof typeof promptHandlers];
