  ...
  private setupRoutes() {
    ...
    this.app.get(this.options.path || "/sse", async (req, res) => {
      try {
        ...
        const transport = new SSEServerTransport("/message", res, {
          allowedOrigins:
            this.options.corsOrigin === true
              ? undefined
              : Array.isArray(this.options.corsOrigin)
    ...
    this.app.post("/message", async (req, res) => {
      try {
        ...
        logInfo(
          `POST /message - SessionId: ${sessionId}, Active sessions: ${Array.from(this.transports.keys()).join(", ")}`,
        );
  ...
  async start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.server = this.app.listen(port, host, () => {
        ...
        logInfo(
          `SSE endpoint available at http://${host}:${port}${this.options.path || "/sse"}`,
        );
        resolve();
      });
