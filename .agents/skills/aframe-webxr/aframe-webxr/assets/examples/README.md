...
<script>
  AFRAME.registerComponent("vr-inventory", {
    ...
    init: function () {
      ...
      // Controller events
      document
        .querySelector('[hand-controls="hand: right"]')
    ...
    prevSlot: function () {
      this.selectedSlot =
        (this.selectedSlot - 1 + this.items.length) % this.items.length;
      this.updateUI();
    },
    ...
    updateUI: function () {
      ...
      slots.forEach((slot, i) => {
        slot.setAttribute(
          "color",
          i === this.selectedSlot ? "#4FC3F7" : "#333",
        );
      });
    },
  });
...
<script>
  ...
  locations.forEach((loc, index) => {
    ...
    hotspot.setAttribute(
      "position",
      `${Math.cos(index * 2) * 3} 1 ${Math.sin(index * 2) * 3}`,
    );
...
<script>
  AFRAME.registerComponent("gesture-detector", {
    ...
    isCircularMotion: function () {
      ...
      const radii = this.positions.map((pos) => pos.distanceTo(center));
...
<script>
  ...
  NAF.schemas.add({
    ...
    components: ["position", "rotation", "scale", "material"],
  });
...
<a-scene physics="debug: false; gravity: -9.8">
  ...
  <a-sphere id="pushButton" position="2 1 -3" radius="0.5" color="#FF0000">
  </a-sphere>
