...
function createDemoObjects() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const radius = 4;
    entity.setPosition(Math.cos(angle) * radius, 1, Math.sin(angle) * radius);
    ...
    entity.rotationSpeed = new pc.Vec3(10 + i * 5, 20 + i * 5, 15 + i * 5);
...
function updateStats() {
  ...
  document.getElementById("triangles").textContent =
    Math.round(stats.misc.tricount / 1000) + "k";
}
