...
>
  <div class="site-shell">
    <div class="grid gap-8 lg:grid-cols-12 lg:items-stretch lg:gap-10">
      <figure
        class="relative min-h-[22rem] overflow-hidden rounded-xl sm:min-h-[30rem] lg:col-span-7"
        data-event-media
      >
        <img
          src={event.image}
      ...
      <div
        class="flex flex-col justify-between border-t border-brand-dark/18 pt-5 lg:col-span-5 lg:py-2"
      >
        <div data-event-copy>
          <p
          ...
          <p
            class="mt-4 font-mono text-[0.6rem] uppercase tracking-[0.13em] text-brand-dark/48"
          >
            {event.date}
          </p>
          <h2
            class="mt-8 max-w-[13ch] font-display text-[clamp(2.4rem,4.2vw,4.6rem)] leading-[0.94] tracking-[-0.035em]"
          >
            {event.title}
          </h2>
        </div>

        <p class="text-base leading-relaxed text-brand-dark/68 sm:text-lg">
          {event.summary}
        </p>

        <dl
          class="mt-8 grid grid-cols-2 gap-5 border-t border-brand-dark/16 pt-5"
        >
        ...
        <ul
          class="mt-6 flex flex-wrap gap-x-5 gap-y-2"
          aria-label="Event focus areas"
        >
