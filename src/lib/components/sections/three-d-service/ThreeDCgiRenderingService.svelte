<script lang="ts">
  ...
  import type { ThreeDCgiSectionData } from "$lib/content/three-d-modeling";

  let { data }: { data: ThreeDCgiSectionData } = $props();
  ...
  onMount(() => {
    ...
    // Three.js 3D Turntable: Luxury Designer Eyewear (Virtual Studio & CGI Lighting)
    ...
    if (canvasContainer) {
      ...
      camera.position.set(0, 0.15, getResponsiveDistance(width / height));
      camera.lookAt(0, 0, 0);
      ...
      // Studio Lighting for Luxury Eyewear, Iridescent Lenses & Metallic Temples
      ...
      // Load Real 3D Model: Luxury Designer Eyewear (Virtual Studio & CGI Lighting)
...
<section
  ...
  id={data.id || "cgi-rendering"}
  class="relative isolate overflow-hidden bg-brand-light py-20 text-brand-dark sm:py-28"
>
  <div id="cgi-rendering" class="absolute -top-24 pointer-events-none"></div>
  <div class="site-shell relative z-10">
    <div class="grid items-center gap-10 lg:grid-cols-12 lg:gap-14">
      <!-- Left Column: Direct 3D Product Turntable (Luxury Designer Eyewear - Virtual Studio & Ray-Traced Reflections) -->
      <div
        class="cgi-reveal order-2 lg:order-1 lg:col-span-7 flex items-center justify-center"
      >
        <div
