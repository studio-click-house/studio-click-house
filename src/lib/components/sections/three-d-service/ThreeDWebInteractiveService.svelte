<script lang="ts">
  ...
  import type { ThreeDTurntablesSectionData } from "$lib/content/three-d-modeling";

  let { data }: { data: ThreeDTurntablesSectionData } = $props();
  ...
  onMount(() => {
    ...
    // Three.js 3D Turntable: Real E-Commerce Vintage Audio BoomBox (Web 3D & 360 Spinners)
    ...
    if (canvasContainer) {
      ...
      // Studio Lighting for Brushed Titanium Metal & Leather
      ...
      // Load Real 3D Model: Luxury Titanium Studio Headphones (E-Commerce 3D & 360 Spinners)
      ...
      loader.load(
        ...
        (gltf) => {
          ...
          // Center geometric pivot with slight upward offset so bottom ear cups never clip
          loadedModel.position.x = -center.x;
          loadedModel.position.y = -center.y + 0.05;
          loadedModel.position.z = -center.z;

          // Comfortable fill ratio (~72% of viewport) with safe margins at any rotation angle
          const radiusXZ = Math.sqrt((size.x / 2) ** 2 + (size.z / 2) ** 2);
          const effectiveDimension = Math.max(radiusXZ * 2, size.y);
          const scale = 2.35 / effectiveDimension;
      ...
      function onPointerMove(e: MouseEvent | TouchEvent) {
        ...
        targetRotationX = Math.max(-0.20, Math.min(0.24, targetRotationX));
      ...
      function animate() {
        ...
        modelGroup.position.y = Math.sin(time) * 0.012;
...
>
  <div class="site-shell relative z-10">
    <div class="grid items-center gap-10 lg:grid-cols-12 lg:gap-14">
      ...
      <!-- Right Column: Direct 3D Product Turntable (Luxury Titanium Studio Headphones - Balanced & Centered) -->
      <div class="turntable-reveal lg:col-span-7 flex items-center justify-center">
        <div class="relative w-full max-w-[740px] aspect-[16/11] sm:aspect-[4/3] min-h-[440px] sm:min-h-[540px] select-none flex items-center justify-center">
          <div
